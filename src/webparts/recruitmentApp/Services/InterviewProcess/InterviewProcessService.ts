    ...
    try {
      ...
      const formattedItems = await Promise.all(
        candidateItems.map(async (item) => {
          ...
          await this.HRMSCandidateScoreCard(filter, filterConditions, item.ID)
            .then((data) => {
              positionResult = data;
            })
            .catch((error) => {
          ...
          const lastCandidateGPA =
            positionResult?.data && positionResult.data.length > 0
              ...
              : null;
          const roundedGPA = lastCandidateGPA
            ? roundGPA(lastCandidateGPA)
            : null;
  ...
  async HRMSCandidateScoreCard(
    ...
    try {
      ...
      await SPServices.SPReadItems({
        ...
        .then((data) => {
          emailToAuthorMap = data.reduce((acc, item) => {
            acc[item.EmailId] = {
        ...
        .catch((error) => {
          console.error("Error fetching sage list details:", error);
        });
