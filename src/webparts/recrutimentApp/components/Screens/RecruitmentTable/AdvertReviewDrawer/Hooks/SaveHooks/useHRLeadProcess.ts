...
export const useHRLeadProcess = (
  form: IDptData,
  currentRoleID: number,
  onemDocs: IDocFiles[],
  BgvData: CheckboxGroupOption[],
) => {
  ...
  const handleHRLeadProcess = useCallback(
    async (finalize: (msg: string, type?: string) => void) => {
      ...
      const JobBasedBGVVerification = serialize(BgvData, (i) => ({
      ...
      const bgvData = BgvData.filter((i: any) => i.checked).map((i: any) => ({
