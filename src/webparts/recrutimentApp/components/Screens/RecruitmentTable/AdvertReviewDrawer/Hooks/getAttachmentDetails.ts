...
export const useAttachmentDetails = (jobId: string, options?: UseAttachmentDetailsOptions) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const [
        ...
        CommonServices.GetAttachmentToLibrary(DocumentLibraray.RoleProfileMaster, jobId, RoleProfileMaster.RoleProfile),
        CommonServices.GetAttachmentToLibrary(DocumentLibraray.RoleProfileMaster, jobId, RoleProfileMaster.Grading),
        CommonServices.GetAttachmentToLibrary(DocumentLibraray.RecruitmentAdvertisementDocument, jobId),
        CommonServices.GetAttachmentToLibrary(DocumentLibraray.ONAMSignedStampDocuments, jobId),
      ]);
      const mappedData: AttachmentDetails[] = [
        {
          title: "Role Profile Document",
          type: "PDF" as const,
          versions: RoleProfileDocment.data.English.map((d: IDocFiles) => ({
            lang: "EN",
            label: d.name,
            content: d.content,
          })).concat(RoleProfileDocment.data.French.map((d: IDocFiles) => ({
            lang: "FR",
            label: d.name,
            content: d.content,
          }))),
        },
        {
          ...
          versions: GradingDocument.data.English.map((d: IDocFiles) => ({
            ...
            label: d.name,
            content: d.content,
          })).concat(GradingDocument.data.French.map((d: IDocFiles) => ({
            lang: "FR",
            label: d.name,
            content: d.content,
          }))),
