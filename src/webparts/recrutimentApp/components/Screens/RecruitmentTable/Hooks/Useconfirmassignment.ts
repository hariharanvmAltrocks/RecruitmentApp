...
import { CommonServices, masterService, RecruitmentServices } from "../../../../services/ServiceExport";
import { PostAgentData, PostRecuritmentData } from "../../../../services/RecruitmentTable/IRecruitmentService";
...
export const useConfirmAssignment = (handleClosePopup: () => void, handleRefresh: () => void) => {
                ...
                if (isHRLead) {
                    ...
                    if (batchResponse.status === ResponeStatus.SUCCESS) {
                        showModal({
                            ...
                            message: payload.vacancies.length > 1 ? RecuritmentHRMsg.HRSuccess : RecuritmentHRMsg.SingleHRSuccessMsg,
                    ...
                    const batchResponse =
                        await RecruitmentServices.InsertExternalAgencyDetails(batchPayloads);

                    if (batchResponse.status === ResponeStatus.SUCCESS) {
                        showModal({
                            type: "success",
                            title: "Agent Assignment Successfully",
                            message: payload.vacancies.length > 1 ? RecuritmentHRMsg.AgencySucess : RecuritmentHRMsg.SingleAgencyMsg,
                            confirmLabel: "Go to Dashboard",
                            onConfirm: () => {
                                closeModal();
                                handleClosePopup();
