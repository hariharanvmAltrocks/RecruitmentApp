...
import { Download, EyeIcon, FileText, Paperclip, View, X } from "lucide-react";
...
const getVersionUrl = (version: { fileUrl?: string; content?: string }) =>
  version.fileUrl || version.content || "";

export const RequiredAttachments: React.FC<RequiredAttachmentsProps> = ({
  attachments,
  isLoading,
}) => {
  ...
  const attachmentCards = useMemo(
    () =>
      attachments.map((doc, index) => (
        <div
          key={`${doc.title}-${index}`}
          className="advert-review-drawer__attachment-card"
        >
          <div className="advert-review-drawer__attachment-header">
            <div
              className={`advert-review-drawer__attachment-type advert-review-drawer__attachment-type--${doc.type.toLowerCase()}`}
            >
              <FileText size={14} />
              <span>{doc.type}</span>
            </div>
            <div className="advert-review-drawer__attachment-meta">
              <div
                className="advert-review-drawer__attachment-title"
                title={doc.title}
              >
                {doc.title}
              </div>
          ...
          <div className="advert-review-drawer__attachment-body">
            {doc.versions.map((version, idx) => {
              const fileUrl = getVersionUrl(
                version as { fileUrl?: string; content?: string },
              );

              return (
                <div
                  key={`${version.lang}-${idx}`}
                ...
                >
                  <div
                    className={`advert-review-drawer__attachment-lang advert-review-drawer__attachment-lang--${version.lang.toLowerCase()}`}
                  >
                    {version.lang}
                  </div>
    ...
    <>
      ...
      {viewerFile && (
        <div className="attachment-viewer" role="presentation">
          ...
          <div
            className="attachment-viewer__panel"
            role="dialog"
            aria-modal="true"
            aria-label={viewerFile.title}
          >
            <div className="attachment-viewer__header">
              <div className="attachment-viewer__meta">
                ...
                <div className="attachment-viewer__subtitle">
                  {viewerFile.label} � {viewerFile.lang}
                </div>
              </div>
