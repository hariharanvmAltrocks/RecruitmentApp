...
export async function getEvaluationFormData(
  ...
  try {
    ...
    if (!currentUserPanel) {
      console.warn(
        "[getEvaluationFormData] Panel emails list:",
        (panelRows as any[]).map((p: any) => p.InterviewPanel?.EMail),
      );
      console.warn(
        "[getEvaluationFormData] Panel Ids list:",
        (panelRows as any[]).map((p: any) => p.InterviewPanel?.Id),
      );
      console.warn(
        "[getEvaluationFormData] Expected currentUserGuid:",
        currentUserGuid,
        "(type:",
        typeof currentUserGuid,
        ")",
      );
    }
    ...
    const [gradeRes, questionsResult] = await Promise.all([
      ...
      jobCodeId
        ? _master
            ...
            .catch((e: any) => {
              console.warn(
                "[getEvaluationFormData] questions fetch failed:",
                e,
              );
              return [];
            })
...
export async function submitScorecard(
  ...
  try {
    ...
    const level1Panels = updatedPanelRows.filter(
      (p: any) => p.InterviewLevel === InterviewLevels.Level1,
    );
    ...
    if (level1Panels.length > 0 && uploadedCount === level1Panels.length) {
      ...
      if (params.jobRequestId) {
        try {
          ...
          console.warn(
            "[submitScorecard] Portal UpdateCandidateStatus failed (non-fatal):",
            apiErr,
          );
        }
      } else {
      ...
      await SPServices.SPUpdateItem({
        ...
        RequestJSON: {
          ...
          // ItemCreated: 'Yes',
          StatusId:
            params.level === InterviewLevels.Level1
      ...
      console.log(
        "[submitScorecard] Candidate SP record updated — HOD workflow complete",
      );
    } else {
      console.log(
        "[submitScorecard] Not all Level1 panels submitted yet — HOD workflow NOT triggered",
      );
    }
    ...
    return {
      success: true,
      message:
        "The Candidate has been Interviewed and Scorecard Submitted successfully.",
    };
  } catch (error) {
  ...
  try {
    const [currentUserGuid, panelRows] = await Promise.all([
      ...
      SPServices.SPReadItems({
        Listname: ListNames.HRMSInterviewPanelDetails,
        Select:
          "ID,InterviewPanel/Id,InterviewPanel/EMail,IsScoreSheetUploaded",
    ...
    if (!userPanel) {
      console.warn(
        "[checkIsAlreadySubmitted] Current user not found in panel rows — treating as not submitted",
      );
      return false;
    }
