...
export function useSubmitReviewScoreCard(
  deps: SubmitHookDeps,
): UseSubmitReviewScoreCardReturn {
  const {
    reviewingCandidate,
    reviewData,
    hodDecision,
  ...
  const submitDecision = React.useCallback(
    async (roleId: number) => {
      ...
      try {
        const jobRequestId =
          (reviewData?.candidateData as any)?.jobRequestId ?? null;
        ...
        const result = await ReviewScoreCardServicesInstance.submitHODDecision({
          ...
          scoreCardId:
            (reviewData?.candidateData as any)?.level2ScorecardId ?? null,
          jobRequestId,
        ...
        setSubmitError(
          e instanceof Error ? e.message : RecuritmentHRMsg.APIErrorMsg,
        );
      } finally {
        setSubmitting(false);
      }
