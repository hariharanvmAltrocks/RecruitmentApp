...
export function useReviewScorecard(
  ...
  const openReview = React.useCallback(
    async (candidate: ScorecardCandidateRow) => {
      ...
      try {
        const result =
          await ReviewScoreCardServicesInstance.getReviewScoreCardData(
            candidate.id,
            currentUserEmail,
        ...
        if (result.hodDecision) {
          ...
          if (posId) {
            ...
            setSelectedPositionText(
              hod.PositionID?.PositionID || hod.PositionText || "",
            );
          }
        }
        ...
        if (result.positionOptions && result.positionOptions.length > 0) {
          ...
          _shouldFetchPosition(candidate.statusId, department) &&
          candidate.jobCodeID &&
          department
        ) {
          ReviewScoreCardServicesInstance.fetchPositionOptions(
            candidate.jobCodeID,
            department,
          )
  ...
  const openComments = React.useCallback(async () => {
    ...
    try {
      const { level1, level2 } =
        await ReviewScoreCardServicesInstance.fetchComments(
