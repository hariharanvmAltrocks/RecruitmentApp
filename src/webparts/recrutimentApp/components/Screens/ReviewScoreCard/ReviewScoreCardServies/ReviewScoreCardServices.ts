...
export interface CandidateListItem {
  ...
  nationality: string; gender: string; jobCodeID: number; jobCode: string;
  department: string; interviewDate: string; disability: string; jobTitle: string;
}

export interface CommentEntry {
  Id: number | null; Name: string; JobTitleInEnglish: string; JobTitleInFrench: string;
  Department: string; Date: any; RoleName: string; comments: string;
  OverAllEvaluationFeedback?: string; Level?: string;
}

export interface PositionOption { key: number; text: string; }

export interface PanelMember {
  name: string; jobTitle: string; department: string; email: string;
}
...
export interface ReviewScoreCardResult {
  success: boolean; candidateId: number; applicantName: string; nationality: string;
  gender: string; qualification: string; miningExp: string; relevantExp: string;
  interviewDate: string; interviewLevel: string; disability: string;
  conflictsOfInterest: string; positionTitle: string; grade: string;
  recruitmentId: number; jobCodeId: number; jobCode: string; department: string;
  panelMembers: PanelMember[]; currentUserPanelId: number | null;
...
async function _calculateGPA(candidateId: number): Promise<string> {
  try {
    const panels: any[] = await SPServices.SPReadItems({
      ...
      Select: 'ID,InterviewLevel,CandidateID/ID', Expand: 'CandidateID',
    ...
    for (const panel of panels) {
      const sc: any[] = await SPServices.SPReadItems({
        Listname: ListNames.HRMSCandidateScoreCard, Select: '*',
        Filter: [{ FilterKey: 'InterviewPanelIDId', Operator: 'eq', FilterValue: panel.ID }],
      });
      if (sc.length > 0) {
        const s = sc[0];
        sumOverall +=
          (Number(s.RelevantQualification)            || 0) + (Number(s.ReleventExperience) || 0) +
          (Number(s.Knowledge)                        || 0) + (Number(s.EnergyLevel)        || 0) +
          (Number(s.MeetJobRequirement)               || 0) + (Number(s.ContributeTowardsCultureRequried) || 0) +
          (Number(s.Experience)                       || 0) + (Number(s.OtherCriteriaScore) || 0);
        const qData  = _parseJson(s.QuestionJson);
        const qScore = qData.reduce((acc: number, q: any) => acc + (Number(Object.values(q)[0]) || 0), 0);
        sumQuestion += qScore; maxQuestion += qData.length * 3;
      }
    }
    const combined = sumOverall + sumQuestion, maxPossible = maxOverallScore + maxQuestion;
    if (maxPossible <= 0) return '';
    return String(Math.floor((combined / maxPossible) * 5 * 100) / 100);
  } catch (e) { console.warn('[_calculateGPA]', candidateId, e); return ''; }
}
...
async function _getOthersInterviewed(jobCodeID: number): Promise<string> {
  try {
    const rows: any[] = await SPServices.SPReadItems({
      ...
      Select: 'ID', Filter: [{ FilterKey: 'JobCodeId', Operator: 'eq', FilterValue: jobCodeID }],
      Topcount: 100,
    });
...
async function _insertOrUpdateLevel1Comment(
  ...
  try {
    const existing: any[] = await SPServices.SPReadItems({
      Listname: ListNames.HRMSRecruitmentCandidateComments, Select: '*',
    ...
    if (match) {
      ...
      await SPServices.SPAddItem({
        Listname: ListNames.HRMSRecruitmentCandidateComments,
        RequestJSON: { CandidateIDId: candidateId, Comments: comments, RoleId: roleId, ...(level ? { Level: level } : {}) },
      });
    }
...
async function _insertOrUpdateLevel2Comment(
  ...
  try {
    const existing: any[] = await SPServices.SPReadItems({
      Listname: ListNames.HRMSCandidateLevel2ScoreCard, Select: '*',
      Filter: [{ FilterKey: 'CandidateIDId', Operator: 'eq', FilterValue: candidateId }],
    });
    const match = existing.find((i: any) => i.RoleId === roleId);
    if (match) {
...
async function _assignPositionID(p: { positionId: number; candidateId: number; recruitmentID: number; }): Promise<void> {
  try {
    const posRes: any[] = await SPServices.SPReadItems({
      Listname: ListNames.HRMSPositionIDMaster, Select: '*',
    ...
    await SPServices.SPAddItem({
      ...
      RequestJSON: {
        PositionIDId: pos.ID, CandidateIDId: p.candidateId,
        RecruitmentIDId: p.recruitmentID, ItemCreated: 'Yes',
        ActionId: WorkflowAction.Submitted, StatusId: StatusId.Pending,
      },
    });
    await SPServices.SPUpdateItem({
      Listname: ListNames.HRMSPositionIDMaster,
      RequestJSON: { PositionIDStatus: 'Recruitment In Progress' }, ID: pos.ID,
    });
...
class ReviewScoreCardServices {
  ...
  async getCandidatesByRecruitmentId(recruitmentID: number): Promise<CandidateListItem[]> {
    try {
      const res: any[] = await SPServices.SPReadItems({
        ...
        Expand: 'JobCode,RecruitmentID,Status', FilterCondition: 'and',
        Filter: [
      ...
      let defaultGrade = '', defaultLevel = '';
      try {
        const posRes: any[] = await SPServices.SPReadItems({
          Listname: ListNames.HRMSRecruitmentPositionDetails,
          Select: '*,PatersonGrade/PatersonGrade', Expand: 'PatersonGrade',
        ...
        if (defaultGrade) {
          const gr: any[] = await SPServices.SPReadItems({
            Listname: ListNames.HRMSGradeMaster, Select: '*',
            Filter: [{ FilterKey: 'PatersonGrade', Operator: 'eq', FilterValue: defaultGrade }],
          });
          defaultLevel = gr?.[0]?.Levels || '';
        }
      } catch (_) {}
      const enriched = await Promise.all((res || []).map(async (item: any) => {
        const candidateId = item.ID;
        const gpa = await _calculateGPA(candidateId);
        return {
          id: candidateId, recruitmentID: item.RecruitmentID?.ID || recruitmentID,
          fullName: [item.FristName, item.MiddleName, item.LastName].filter(Boolean).join(' ').trim(),
          positionTitle: item.PositionTitle || '', interviewLevel: item.InterviewLevel || defaultLevel,
          grade: item.JobGrade || item.PatersonGrade || defaultGrade || '', gpa,
          ...
          jobCodeID: item.JobCodeId || 0, jobCode: item.JobCode?.JobCode || '',
          department: item.Department || '',
          interviewDate: (item.InterviewDateLevel2 || item.InterviewDate || '').split('T')[0],
          disability: item.Disability || '', jobTitle: item.PositionTitle || '',
        } as CandidateListItem;
      }));
      return enriched;
    } catch (e) { console.error('[getCandidatesByRecruitmentId]', e); return []; }
  }

  async getReviewScoreCardData(
    candidateId: number, currentUserEmail: string, candidate?: any,
    ...
    try {
      ...
      const uniqueEmails: string[] = Array.from(
        new Set((panelRows as any[]).map((p: any) => p.InterviewPanel?.EMail).filter(Boolean))
      );
      ...
      await Promise.all(uniqueEmails.map(async (email: string) => {
        try {
          const r = await _master.GetUserDetails([{ FilterKey: 'EmailId', Operator: 'eq', FilterValue: email }], 'and');
          if (r?.data) {
            const name = [r.data.FirstName, r.data.MiddleName, r.data.LastName].filter(Boolean).join(' ').trim();
            if (name) emailToDetails[email.toLowerCase()] = { name, jobTitle: r.data.JopTitleEnglish ?? '', department: r.data.DepartmentName ?? '' };
          }
        } catch {}
      }));
      ...
      const reviewerName = reviewer ? [reviewer.FirstName, reviewer.MiddleName, reviewer.LastName].filter(Boolean).join(' ').trim() : '';
      ...
      const [gradeRes, questionsResult, scorecardData, level2ScorecardData, hodDecisionData, positionOptionsData, commentsData] =
        await Promise.all([
          grade ? Promise.resolve(null) : _master.GetGradeLevel(raw.JobGrade || jobCodeStr).catch(() => null),
          jobCodeId
            ? _master.GetJobUniqueDataValue(jobCodeId).then(async (r: any) => {
                const key = r?.data?.JobCode ?? ''; if (!key) return [];
                const qRes = await _questApi.getQuestionnaire(key);
                return (qRes?.data ?? []).map((q: any) => ({ id: q.id, question: q.question, answer: q.answer ?? '', rating: null }));
              }).catch(() => [])
            : Promise.resolve([]),
      ...
      return {
        success: true, candidateId, applicantName: fullName, nationality: raw.Nationality ?? '',
        gender: raw.Gender ?? '', qualification: raw.Qualification ?? '',
        miningExp: raw.TotalYearOfExperiance ?? '', relevantExp: raw.ReleventExperience ?? '',
        interviewDate, interviewLevel, disability: raw.Disability ?? raw.disability ?? '',
        conflictsOfInterest: raw.ConflictsOfInterest ?? '', positionTitle: raw.PositionTitle ?? '',
        grade, recruitmentId, jobCodeId, jobCode: jobCodeStr, department, panelMembers,
        currentUserPanelId: currentUserPanel?.ID ?? null, currentUserGuid,
        reviewerName, jobTitleEn, jobTitleFr,
        questions: questionsResult as ReviewScoreCardQuestion[],
        scorecard: scorecardData, level2Scorecard: level2ScorecardData,
  ...
  async _getCandidateScorecard(candidateId: number): Promise<any[] | null> {
    try {
      const panels: any[] = await SPServices.SPReadItems({
        Listname: ListNames.HRMSInterviewPanelDetails,
        Select: 'ID,InterviewPanel/Title,InterviewLevel', Expand: 'InterviewPanel',
        Filter: [{ FilterKey: 'CandidateID/Id', Operator: 'eq', FilterValue: candidateId }],
      });
      const allScores: any[] = [];
      for (const p of panels) {
        ...
        if (sc?.length) allScores.push({ ...sc[0], InterviewPersonName: p.InterviewPanel?.Title || '', InterviewLevel: p.InterviewLevel || '' });
      }
      return allScores.length > 0 ? allScores : null;
    } catch (e) { console.error('[_getCandidateScorecard]', e); return null; }
  }
  ...
  async _getHODDecision(candidateId: number): Promise<any> {
    try {
      const res: any[] = await SPServices.SPReadItems({
        Listname: ListNames.HRMSSelectedCandidateDetailsByHOD,
        Select: '*,PositionID/PositionID,PositionID/ID', Expand: 'PositionID',
        Filter: [{ FilterKey: 'CandidateIDId', Operator: 'eq', FilterValue: candidateId }],
      });
      return res?.[0] ?? null;
    } catch { return null; }
  ...
  async fetchComments(candidateId: number): Promise<{ level1: CommentEntry[]; level2: CommentEntry[] }> {
    try {
      ...
      const panelEmails: string[] = Array.from(new Set((panelItems as any[]).map((p: any) => p.InterviewPanel?.EMail).filter(Boolean)));
      const empMap: Record<string, { name: string; jobTitle: string; jobTitleFr: string; department: string }> = {};
      await Promise.all(panelEmails.map(async (email: string) => {
        try {
          const r = await _master.GetUserDetails([{ FilterKey: 'EmailId', Operator: 'eq', FilterValue: email }], 'and');
          if (r?.data) {
            const d    = r.data;
            const name = [d.FirstName, d.MiddleName, d.LastName].filter(Boolean).join(' ').trim();
            empMap[email.toLowerCase()] = { name, jobTitle: d.JopTitleEnglish || '', jobTitleFr: d.JopTitleFrench || '', department: d.DepartmentName || '' };
          }
        } catch {}
      }));
      ...
      for (const panel of panelItems as any[]) {
        ...
        level1.push({
          Id: panel.ID,
          Name: emp?.name || scorecard?.Author?.Title || panel.InterviewPanel?.Title || '',
          JobTitleInEnglish: emp?.jobTitle   || '', JobTitleInFrench: emp?.jobTitleFr || '',
          Department: emp?.department || '',
          Date: scorecard?.Created ? new Date(scorecard.Created) : null,
          RoleName: scorecard?.Role?.RoleTitle || '',
          comments: feedbackComment, OverAllEvaluationFeedback: overallFeedback, Level: 'Level 1',
        });
      }

      const l2Emails: string[] = Array.from(new Set((level2Items as any[]).map((i: any) => i.Author?.EMail).filter(Boolean)));
      const l2EmpMap: Record<string, { name: string; jobTitle: string; jobTitleFr: string; department: string }> = {};
      await Promise.all(l2Emails.map(async (email: string) => {
        try {
          const r = await _master.GetUserDetails([{ FilterKey: 'EmailId', Operator: 'eq', FilterValue: email }], 'and');
          if (r?.data) {
            const d    = r.data;
            const name = [d.FirstName, d.MiddleName, d.LastName].filter(Boolean).join(' ').trim();
            l2EmpMap[email.toLowerCase()] = { name, jobTitle: d.JopTitleEnglish || '', jobTitleFr: d.JopTitleFrench || '', department: d.DepartmentName || '' };
          }
        } catch {}
      }));

      const level2: CommentEntry[] = (level2Items as any[])
        .filter((i: any) => i.CandidateID?.ID === candidateId || i.CandidateIDId === candidateId)
        .map((i: any): CommentEntry => {
          const email = (i.Author?.EMail ?? '').toLowerCase();
          const emp   = l2EmpMap[email];
          return {
            Id: i.ID ?? null, Name: emp?.name || i.Author?.Title || '',
    ...
    } catch (e) { console.error('[fetchComments]', e); return { level1: [], level2: [] }; }
  }
  ...
  async submitHODDecision(params: HODSubmitParams): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      if (lv2) {
        ...
        const matchingPanels = allPanels.filter((p: any) => p.CandidateID?.ID === candidateId);
        const userPanels     = matchingPanels.filter((p: any) => String(p.InterviewPanel?.Id) === String(currentUserGuid));
        for (const panel of userPanels) {
          await SPServices.SPUpdateItem({
            Listname: ListNames.HRMSInterviewPanelDetails,
            RequestJSON: { IsScoreSheetUploaded: 'Yes' }, ID: panel.ID,
          });
        }
        ...
        const refreshed: any[] = await SPServices.SPReadItems({
          Listname: ListNames.HRMSInterviewPanelDetails,
          Select: 'ID,CandidateID/ID,InterviewLevel,IsScoreSheetUploaded', Expand: 'CandidateID',
          Filter: [{ FilterKey: 'CandidateID/Id', Operator: 'eq', FilterValue: candidateId }],
        });
        console.log('Branch 1 Step 3: Refreshed panels details:', refreshed.map(p => ({ id: p.ID, interviewLevel: p.InterviewLevel })));
        const level2Panels  = refreshed.filter((p: any) => p.InterviewLevel === 'Level 2'); // 'Level 2' matches HRMSInterviewPanelDetails.InterviewLevel
        const uploadedCount = level2Panels.filter((p: any) => p.IsScoreSheetUploaded === 'Yes').length;
        ...
        if (uploadedCount === level2Panels.length && level2Panels.length > 0) {
          await SPServices.SPUpdateItem({
            Listname: ListNames.HRMSRecruitmentCandidatePersonalDetails,
            RequestJSON: { ScoreCardLevelItemCreated: 'Yes', ActionId: WorkflowAction.Approved, ItemCreated: 'Yes' },
            ID: candidateId,
          });
        }
      ...
      if ((hodDecision === 'No' || hodDecision === 'On Hold') && positionId) {
        ...
        await SPServices.SPUpdateItem({
          Listname: ListNames.HRMSPositionIDMaster,
          RequestJSON: { PositionIDStatus: 'Recruitment Initiated' }, ID: positionId,
        });
