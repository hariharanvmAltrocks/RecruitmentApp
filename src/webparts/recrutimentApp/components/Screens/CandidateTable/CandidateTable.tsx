...
export const CandidateTable: React.FC = (props: any) => {
  ...
  const handleAction = useCallback((item: any) => {
    ...
    const getAction = (status: number) => {
      ...
      if (reviewStatuses.includes(item.workflowStatusId)) return ActionID.Review;
      if (holdStatuses.includes(item.workflowStatusId)) return ActionID.onHold;
      if (scheduleStatuses.includes(Number(item.workflowStatusId))) return ActionID.schedule;
