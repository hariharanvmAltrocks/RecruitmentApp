...
import { EvalUIConfig, EvalQueryConfig, InterviewLevels, HOD_SCORECARD_STATUSES, StatusId as EvalStatusId, WorkflowAction } from "../config/EvaluationConfig";
import {
  TooltipEntry, ScoreSheetResult, EvaluationCandidate,
  RawScorecard, ScoreRow, CommentEntry, PositionOption,
...
export const evaluationService = {
  ...
  async fetchExistingHODDecision(
    ...
    try {
      ...
      // Match by roleId (same logic as old insertOrUpdateCandidateCommentLevel1)
      const match = items.find(
        (item: any) => item.RoleId === roleId
      ) || items[0]; // fallback to first if no roleId match

      const comments = match?.Comments || "";

      // Fetch assigned position from HRMSSelectedCandidateDetailsByHOD
  ...
  async updateCandidateStatusFull(params: {
    ...
    try {
      ...
      // ── BRANCH 1: Level 2 (InterviewScheduledforLevel2) ──────────────────
      if (statusId === StatusId.InterviewScheduledforLevel2) {
        ...
        // Step 3: Check if ALL level2 panels uploaded → promote candidate
      ...
      // Step 3: WorkflowApi UpdateCandidateStatus
      let workflowStatus = "";
      switch (hodDecision) {
      ...
      // Step 4: CandidateSeletionApi — update SP list directly (mirrors old ActionUpdate)
      let actionId = 0;
      ...
      switch (hodDecision) {
        case "Yes":
          ...
          if (
            ...
            newStatusId = StatusId.PendingwithHODtoAssignPositionID; // 130
          } else {
            newStatusId = StatusId.Selected; // 122
          }
          break;
        case "No":
          actionId    = WorkflowAction.Reject; // 2
          if (statusId === StatusId.PendingwithHODtoselectthecandidate) {
            newStatusId = StatusId.CandidateRejectedbyHODLevel1; // 167
          } else if (statusId === StatusId.PendingwithHODtoselectthecandidateLevel2) {
            newStatusId = StatusId.CandidateRejectedbyHODLevel2; // 168
          } else {
            newStatusId = StatusId.RejectedbyHOD; // 15
          }
          break;
        case "On Hold":
          actionId    = WorkflowAction.OnHold; // 10
          if (statusId === StatusId.PendingwithHODtoselectthecandidate) {
            newStatusId = StatusId.CandidateOnHoldbyHODLevel1; // 165
          } else if (statusId === StatusId.PendingwithHODtoselectthecandidateLevel2) {
            newStatusId = StatusId.CandidateOnHoldbyHODLevel2; // 166
          } else {
            newStatusId = StatusId.OnHoldbyHOD; // 123
          }
          break;
      }
  ...
  async fetchPositionOptions(jobCodeID: number | string, department: string): Promise<PositionOption[]> {
    try {
      // Dynamic fetch from HRMSPositionIDMaster (mirrors old PositionData() in HodViewScorecard)
      ...
      // Fallback: just filter by department if no results
