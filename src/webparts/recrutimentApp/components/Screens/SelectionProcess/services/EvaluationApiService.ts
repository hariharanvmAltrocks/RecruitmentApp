...
export const evaluationService = {
  ...
  async getGradeAndLevel(
    recruitmentID: number,
  ): Promise<{ grade: string; level: string; jobCodeID: number }> {
    let grade = "";
    let level = "";
    let jobCodeID = 0;
    try {
  ...
  async checkScoreSheet(
    ...
    try {
      ...
      if (statusId === StatusId.InterviewScheduled) {
        ...
        return already
          ? { canProceed: false, level: InterviewLevels.Level1 }
          : { canProceed: true };
      }
      if (statusId === StatusId.InterviewScheduledforLevel2) {
        const already = userPanels
          ...
          .some((p: any) => p.IsScoreSheetUploaded === "Yes");
        return already
          ? { canProceed: false, level: InterviewLevels.Level2 }
          : { canProceed: true };
      }
    ...
    try {
      ...
      try {
        const sageRes: any[] = await SPServices.SPReadItems({
          Listname: ListNames.HRMSSageList,
          Select:
            "*,JobTitleInEnglish/JobTitleInEnglish,JobTitleInFrench/JobTitleInFrench",
      ...
      if (jobUniqueKey) {
        try {
          const qResponse =
            await questionnaireService.getQuestionnaire(jobUniqueKey);
    ...
    try {
      if (
        ...
        return {
          success: false,
          message:
            "Required fields: candidateId, statusId, hodDecision, comments",
        };
      }
      ...
      switch (hodDecision.trim().toLowerCase()) {
        ...
        case "on hold":
          ...
          break;
        default:
          throw new Error(`Invalid HOD decision: ${hodDecision}`);
      }
      ...
      return {
        ...
        data: { candidateId, statusId },
      };
    } catch (error: any) {
      return {
        success: false,
        message: `Error updating candidate status: ${error?.message}`,
      };
    }
  },
  ...
  async fetchExistingHODDecision(
    ...
    try {
      ...
      const match =
        items.find((item: any) => item.RoleId === roleId) || items[0];
      ...
      try {
        ...
        if (posList && posList.length > 0) {
          ...
          positionText =
            posItem.PositionID?.PositionID || posItem.PositionIDText || "";
        }
      } catch (_) {}
  ...
  async updateCandidateStatusFull(params: {
    ...
    try {
        ...
        if (currentUserGuid) {
          ...
          const userPanels = matchingPanels.filter(
            (p: any) =>
              ...
              (p.CandidateID?.ID ?? p.CandidateID?.Id ?? p.CandidateIDId) ===
                candidateId,
          );
      ...
      try {
        ...
        console.warn(
          "[updateCandidateStatusFull] WorkflowApi call failed (non-fatal):",
          wfErr,
        );
      }
      let actionId = 0;
      ...
      switch (hodDecision) {
        case "Yes":
          ...
          if (
            ...
            newStatusId = StatusId.PendingwithpositionIDAssignmentWithHOD;
          } else {
  ...
  async fetchScorecardJobList(
    currentUserEmail: string = "",
  ): Promise<ScorecardJobRow[]> {
    try {
      ...
      return res.map((item: any) => ({
        ...
        businessUnitCode:
          ...
          item.BusinessUnitCode?.Title ||
          item.BusinessUnitCode ||
          "",
        positionRequest:
          item.Type || item.PositionRequest || "New Position Request",
      }));
    } catch (e) {
  ...
  async fetchScorecardCandidates(
    ...
    try {
      const filter: any[] =
        candidateFilter && candidateFilter.length
          ? [...candidateFilter]
          : [
              { FilterKey: "ItemCreated", Operator: "eq", FilterValue: "No" },
              {
      ...
      const filteredRes = res.filter(
        (item) =>
          item.RecruitmentID?.ID === recruitmentID ||
      ...
      const { grade: defaultGrade } =
        await this.getGradeAndLevel(recruitmentID);
            ...
            for (const panel of panels) {
              ...
              if (scorecards.length > 0) {
                ...
                const questionScore = questionData.reduce(
                  (sum: number, q: any) =>
                    sum + (Number(Object.values(q)[0]) || 0),
                  0,
                );
  ...
  async fetchPanelByLevel(
    ...
    try {
      const panels: any[] = await SPServices.SPReadItems({
        Listname: ListNames.HRMSInterviewPanelDetails,
        Select:
          "InterviewLevel,InterviewPanel/Id,InterviewPanel/Title,InterviewPanel/EMail",
        Expand: "InterviewPanel",
        Filter: [
          {
            FilterKey: "CandidateID/Id",
      ...
      for (const p of panels || []) {
        ...
        const name =
          emailToName[p.InterviewPanel?.EMail?.toLowerCase() || ""] ||
          p.InterviewPanel?.Title ||
          "";
  ...
  async fetchComments(
    ...
    try {
      const panelItems: any[] = await SPServices.SPReadItems({
        Listname: ListNames.HRMSInterviewPanelDetails,
        Select:
          "ID, CandidateID/ID, InterviewLevel, InterviewPanel/Id, InterviewPanel/Title, InterviewPanel/EMail",
        Expand: "InterviewPanel, CandidateID",
        Filter: [
          {
            FilterKey: "CandidateID/Id",
      ...
      const scoreItems: any[] = await SPServices.SPReadItems({
        Listname: ListNames.HRMSCandidateScoreCard,
        Select:
          "InterviewPanelID/ID, Feedback, OverAllEvaluationFeedback, Role/RoleTitle, InterviewPersonName/Title, Author/Title, Author/EMail, Created, QuestionJson, RecruitmentID/ID",
        Expand:
          "InterviewPanelID, Role, InterviewPersonName, Author, RecruitmentID",
        FilterCondition: "and",
        Filter: [
          {
      ...
      const authorEmails: string[] = Array.from(
        new Set(
          (scoreItems || []).map((sc: any) => sc.Author?.EMail).filter(Boolean),
        ),
      );
          ...
          await Promise.all(
            authorEmails.map(async (email: string) => {
              const sage: any[] = await SPServices.SPReadItems({
                Listname: ListNames.HRMSSageList,
                Select:
                  "EmailId, FirstName, MiddleName, LastName, JobTitle, JobTitleInEnglish, JobTitleInFrench, Department, DepartmentName",
                Filter: [
                  { FilterKey: "EmailId", Operator: "eq", FilterValue: email },
                ],
              });
      ...
      for (const panel of panelItems || []) {
        ...
        const jobTitleEn =
          employee?.JobTitleInEnglish || employee?.JobTitle || "";
      ...
      const l2Items: any[] = await SPServices.SPReadItems({
        ...
        Select:
          "ID, CandidateID/ID, CandidateID/Title, Comments, Role/ID, Role/RoleTitle, Level, Author/Title, Author/EMail, Created",
      ...
      const l2AuthorEmails: string[] = Array.from(
        new Set(
          ...
          await Promise.all(
            l2AuthorEmails
              ...
              .map(async (email: string) => {
                ...
                if (sage?.length)
                  l2EmailToEmployee[email.toLowerCase()] = sage[0];
              }),
          );
        } catch (_) {}
      }
    ...
    try {
      const filterConditions = [
        ...
        { FilterKey: "Department", Operator: "eq", FilterValue: department },
        {
          ...
          Operator: "eq",
          FilterValue: "Recruitment Initiator",
        },
      ];
...
async function _insertOrUpdateLevel2Comment(
  ...
  try {
    ...
    const match = existing.find(
      (item: any) =>
        item.CandidateID?.ID === candidateId && item.RoleId === roleId,
    );
...
export const EvaluationServiceHelper = {
  buildRow(
    ...
    const formattedLevel =
      level === InterviewLevels.Level2 ? InterviewLevels.Levels2 : level;
    const interviewDateTime = rawDate
    ...
    return {
      ...
      statusId:
        ...
        "",
      recruitmentID:
        candidate.RecruitmentID?.ID || candidate.RecruitmentID || "",
      jobCodeID,
    };
  },
};
