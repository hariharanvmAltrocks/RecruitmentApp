...
export default class CareerPortalService implements Icareerportal {
  async UpsertJobs(
    ...
    try {
      ...
      const response =
        await postAdveDetails.postUpsertJobs(AdvertisementDetails);
      return {
        data: response.data,
  ...
  async getCandidateProfile(
    CandidateID: string,
    EmployeeList?: any[],
    RecrutimentData?: DataSyncToRecruitmentResponse,
    ...
    try {
      ...
      await getProfileData
        ...
        .then(async (res) => {
          ...
          const [RoleProfileDocment, AdvertismentDocment] = await Promise.all([
          ...
          const EmployeeHR = EmployeeList?.find((options: any) => {
            return (
              options.Email?.toLowerCase() === RecrutimentData?.AssignEMail
            );
          });
          ...
          const EmployeeLM = EmployeeList?.find((options: any) => {
            return (
              ...
              RecrutimentData?.AssignLineManager
            );
          });
  ...
  async getQuestionnaire(
    jobCode: string,
  ): Promise<ApiResponse<QuestionItem[] | null>> {
    try {
      ...
      const GetQuestionnaire: QuestionItem[] = response.data.data.map(
        (item: any, index: number) => {
          ...
          return {
            ...
            answer:
              item?.question?.questionXAnswers?.[0]?.optContent?.contentEn ??
              "",
            answerFr:
              item?.question?.questionXAnswers?.[0]?.optContent?.contentFr ??
              "",
            rating: 0,
      ...
      const GetQuestionnaire: ViewQuestion[] = response.data.data
        .map((item: any, index: number) => {
          ...
          const expectedAnswer = item?.question?.questionXAnswers.map(
            (item: any) => item?.optContent?.contentEn,
          );
          const expectedAnswerFr = item?.question?.questionXAnswers.map(
            (item: any) => item?.optContent?.contentFr,
          );
  ...
  async GetStateByCountry(
    code: string,
  ): Promise<ApiResponse<GetMasterByCountry[] | null>> {
    try {
      const response = await GetStateByCountryApi.GetStatebyCountry(code);
      const GetAllMasterData: GetMasterByCountry[] = response.data?.data?.map(
        (item: any) => ({
          id: item.stateId,
          code: item.stateId,
          text: item.state,
  ...
  async UpsertProfile(
    ...
    try {
      ...
      const UpsertData =
        await UploadCandidateCVData.UpsertProfile(UpsertProfile);
      if (UpsertData.status === ResponeStatus.SUCCESS) {
        ...
        if (Document.familyLink.length > 0) {
          FamilyLink = await CareerPotalServices.UpsertDocumentUpload({
            DocumentTypeEnum: "0",
        ...
        if (Document.businessLink.length > 0) {
          BusinessLink = await CareerPotalServices.UpsertDocumentUpload({
            DocumentTypeEnum: "0",
        ...
        if (
          CandidateCV.status === ResponeStatus.SUCCESS &&
          FamilyLink.status === ResponeStatus.SUCCESS &&
          BusinessLink.status === ResponeStatus.SUCCESS
        ) {
          ...
          const profileCurrentPosition = data.profileDetailExperiences.map(
            (item) => {
              return {
                ...item,
  ...
  async UpsertDocumentUpload(
    DocumentDetails: DocumentValue,
  ): Promise<ApiResponse<any | null>> {
    try {
  ...
  async CheckMyCandidateAppliedJobs(
    data: CheckMyCandidate,
  ): Promise<ApiResponse<any | null>> {
    try {
      ...
      const response =
        await UploadCandidateCVData.CheckMyCandidateAppliedJobs(CheckCandidate);
  ...
  async GetCandiateForJobs(
    JobCode: string,
    FilterValue: GetProfileByFilter,
  ): Promise<ApiResponse<GetProfileByJobCode[] | null>> {
    try {
      let GetProfileByJobCodeData: GetProfileByJobCode[] = [];
      await UploadCandidateCVData.GetCandiateForJobs(JobCode, FilterValue)
        .then((res) => {
          const TotalItems = res?.data?.pagination?.totalItems;
          GetProfileByJobCodeData = res.data.data.map(
            (item: any, index: number) => {
  ...
  async GetQuestionByJobCode(
    jobCode: string,
  ): Promise<ApiResponse<CommanQuestion[] | null>> {
    try {
      const response =
        await UploadCandidateCVData.GetQuestionByJobCode(jobCode);
      const getQueAnswers = response.data.data.jobCommonQuestions
        .filter((item: any) => item?.scopeId === "S7")
        .sort((a: any, b: any) => {
          const textA = typeof a.text === "string" ? a.text : "";
          const textB = typeof b.text === "string" ? b.text : "";
          return textA.localeCompare(textB);
        });
      const GetQuestionnaire: CommanQuestion[] = getQueAnswers.map(
        (item: any, index: number) => {
          const incrementedIndex = index + 1;
          ...
          const plainText = tempElement.innerText.replace(/\s*\*$/, "").trim();
        ...
        .then(async (res: any) => {
          ...
          for (const item of res) {
            if (item?.LineManagerId && item?.LineManager?.EMail) {
              ...
              const UserName = await CommonServices.GetUserName(
