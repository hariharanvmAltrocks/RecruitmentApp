...
export type ICommonService = {
  ...
  getUserIDByEmail(userId: number): Promise<ApiResponse<any | null>>;
  GetMasterData(ListName: string,Filter?: any[]): Promise<ApiResponse<any[]>>;
  GetSageMasterData(ListName: string, Filter: any[]): Promise<ApiResponse<any[]>>;
  ...
  GetUserName(email: string): Promise<ApiResponse<any | null>>;
  uploadAttachmentToLibrary(
    PositionCode: string,
    AttachFile: IDocFiles[],
    Listname: string,
  ): Promise<ApiResponse<any>>;
  PostCommanDataInsert(obj: any, ListName: string): Promise<ApiResponse<any>>;
};
