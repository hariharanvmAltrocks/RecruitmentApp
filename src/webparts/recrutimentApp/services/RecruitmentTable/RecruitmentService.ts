...
export default class RecruitmentService implements IRecruitmentService {
    ...
    try {
      ...
      const postResults = await Promise.all(
        payloads.map(async (item) => {
          const agentCode = agentMasterMap.get(item.Data.AgentId);
          ...
          try {
            await postAdveDetails.postAgenciesJobs(AgentDetails);
      ...
      if (withComments.length) {
        ...
        for (const { item } of withComments) {
          void commentList.items.add({
            ...item.CommentsList,
            RecruitmentIDId: item.Data.RecrutimentId,
          });
        }
      ...
      return {
        data: succeeded.map((r) => r.item),
        status: succeeded.length > 0 ? 200 : 500,
        message: `Processed ${succeeded.length}/${payloads.length} record(s) successfully`,
      };
    } catch (error) {
      console.error("InsertExternalAgencyDetails error:", error);
      return { data: [], status: 500, message: "Batch insert failed" };
    }
  }
  ...
  async UploadAdvertisementInPortal(
    ...
    try {
      ...
      if (JobBasedBGVVerification && roleProfileList[0].ID) {
