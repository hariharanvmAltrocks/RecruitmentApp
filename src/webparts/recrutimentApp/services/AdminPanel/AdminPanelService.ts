import { AdminCreateUser, ExternalUserDetails } from "../../models/adminpanel";
...
import { AdminPanelServiceApi } from "../AxiosService/CareerPortalAPI";
import SPServices from "../SPService/spservice";
import { AdminDashboard, IAdminPanelService } from "./IAdminpanelService";

export default class AdminPanelService implements IAdminPanelService {
  async getAdminPanelDashboard(
    FilterValue: AdminPItem,
  ): Promise<ApiResponse<AdminDashboard[] | null>> {
    try {
      let GetProfileByJobCodeData: AdminDashboard[] = [];
      await AdminPanelServiceApi.GetAdminPanelDashboard(FilterValue)
        .then((res) => {
  ...
  async UpsertExternalUser(
    ...
    try {
      ...
      const response =
        await AdminPanelServiceApi.UpsertExternalUser(UpsertUserDetails);
