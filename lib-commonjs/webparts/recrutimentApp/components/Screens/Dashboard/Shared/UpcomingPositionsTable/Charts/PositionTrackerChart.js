...
var PositionTrackerChart = function (_a) {
    ...
    var itemsPerPage = 4;
    var rawChartData = (0, react_1.useMemo)(function () { return data || []; }, [data]);
    var totalPages = Math.ceil(rawChartData.length / itemsPerPage);
    var safeCurrentPage = Math.max(1, Math.min(currentPage, totalPages || 1));
    var chartData = (0, react_1.useMemo)(function () {
        return rawChartData.slice((safeCurrentPage - 1) * itemsPerPage, safeCurrentPage * itemsPerPage);
    ...
    return (react_1.default.createElement(Card_1.default, { className: PositionTrackerChart_module_scss_1.default.chartCard },
        ...
        totalPages > 1 && (react_1.default.createElement("div", { className: PositionTrackerChart_module_scss_1.default.pagination },
            react_1.default.createElement("button", { type: "button", className: PositionTrackerChart_module_scss_1.default.pageArrow, onClick: function () { return setCurrentPage(function (prev) { return Math.max(1, prev - 1); }); }, disabled: safeCurrentPage === 1, "aria-label": "Previous Page" }, "\u2039"),
            pages.map(function (p) { return (react_1.default.createElement("button", { key: p, type: "button", className: "".concat(PositionTrackerChart_module_scss_1.default.pageNumber, " ").concat(p === safeCurrentPage ? PositionTrackerChart_module_scss_1.default.active : ""), onClick: function () { return setCurrentPage(p); } }, p)); }),
            react_1.default.createElement("button", { type: "button", className: PositionTrackerChart_module_scss_1.default.pageArrow, onClick: function () { return setCurrentPage(function (prev) { return Math.min(totalPages, prev + 1); }); }, disabled: safeCurrentPage === totalPages, "aria-label": "Next Page" }, "\u203A")))));
};
